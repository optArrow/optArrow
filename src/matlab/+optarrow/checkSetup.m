function report = checkSetup(endpoint, opts)
% optarrow.checkSetup  Verify the OptArrow Gateway is reachable and select
% the available MATLAB transport.
%
% Native Arrow IPC is preferred. If the "MATLAB Interface to Apache Arrow"
% add-on is not available or is incompatible with the MATLAB runtime, the
% OptArrow MATLAB client falls back to the Gateway JSON route.
%
% USAGE:
%
%   report = optarrow.checkSetup()
%   report = optarrow.checkSetup(endpoint)
%   report = optarrow.checkSetup(endpoint, opts)
%
% INPUTS:
%   endpoint   char/string  Gateway URL (default: 'http://127.0.0.1:8000/cobra/compute').
%
%   opts       struct  Optional:
%                timeoutSec     numeric  Connection timeout in seconds (default: 10)
%                throwOnError   logical  Throw on failure (default: true)
%
% OUTPUT:
%   report   struct with fields:
%              ok            logical  true if all checks passed
%              endpoint      char     resolved endpoint URL
%              timeoutSec    double
%              httpStatus    double   HTTP status code (0 if unreachable)
%              arrowBackend  char     'native' | 'json'
...
% which() finds M-, P-, MEX-, and class files — unlike exist(...,'file')==2.
...
try
    ...
    if contains(ME.message, 'GLIBCXX') || contains(ME.message, 'libstdc++')
        report.arrowBackend = 'json';
        fprintf('   Arrow backend: JSON fallback (libstdc++ mismatch).\n');
        return;
    end
end
